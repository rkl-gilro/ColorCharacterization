...
save_calibration = 'Calibration_UnrealUnlit_HTCVive_14_03_2023_xyz.mat';
...
set(gca,  'FontSize', 30, 'fontname','TeXGyreTermes', 'Color', 'none'); %
grid on
...
for ch=1:3
    ...
    text(x(end-10), (PS(2,ch)*x(end-1).^PS(1,ch)/2), name_channels{ch}, ...
        'FontSize', 30, 'fontname','TeXGyreTermes')
    box off

    leg = legend({'Measured', 'Estimated'}, 'Location','northwest',  'FontSize', 30, 'fontname','TeXGyreTermes');
    
    set(leg,'Box','off','TextColor','k')
...
text(x(end-10), (PS(2,ch)*x(end-1).^PS(1,ch)/2), 'Achromatic', ...
        'FontSize', 30, 'fontname','TeXGyreTermes')
leg = legend({'Additivity', 'Measured'}, 'Location','northwest',  'FontSize', 30, 'fontname','TeXGyreTermes');
set(leg,'Box','off')
box off
xlabel('Normalized Intensity Values','FontSize',15)
ylabel('Luminance (cd/m^{2})','FontSize',15)
...
for co = 1:3
    for ch = 1:4
        if  ch < 4
            ...
            for k = 1:length(y)-1
                d(k) = abs(y(k+1)-y(k))/abs(x(k+1)-x(k));
            end
