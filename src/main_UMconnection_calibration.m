...
%% D:\VR_Projects\TCP_Connection\TCP_project_calibration
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
HTC=0;
Pimax=0;
...
fopen(t);
% 
save_filename = 'Calibration_UnrealStandard_Varjo_23_03_2023.mat';
xCompare=1000;
yCompare=-1;
zCompare=1000;
% 
...
for i = 1:size(PredefinedRGB, 1)
    ...
    fwrite(t, "Value:" + range(i, 1) + "," + range(i, 2) + "," + range(i, 3));
    a = fscanf(t, '%s\n');
    ...
    while(cont<3&&(abs(xyzObtain(1)-xCompare)<=0.8)&&(abs(xyzObtain(2)-yCompare)<=0.8)&&(abs(xyzObtain(3)-zCompare)<=0.8))
        fwrite(t, "Value:" + 1 + "," + 1 + "," + 1);
        a = fscanf(t, '%s\n');
        while ~strcmp(a, "SHOT")
            a = fscanf(t, '%s\n');
...
% save(save_filename, 'Validation_s', ...
%                     'Validation_rand', 'rand_RGB', '-v7.3');
