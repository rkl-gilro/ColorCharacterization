...
save_filename = ['Calibration_UnityUnlit_CS2000_Varjo_dE_23_03_2023.mat'];
...
for ch=1:3
    ...
    % relative error
    est_err(ch) = sum((1-(gammaFun2(PS(:, ch),x)/y)).^2);
end
...
%     plot(x,(PS(2,ch)*x.^PS(1,ch) + PS(3,ch)),[cols{ch} '-'],'LineWidth',linew);

    plot(x, gammaFun2(PS(:, ch), x),[cols{ch} '-'],'LineWidth',linew);
